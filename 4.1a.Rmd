...
# Yes, as different altitudes have different climates. The peninsula maps support this in that 
...
```
```{r}
map('world', fill=T, col="grey", bg="light blue", xlim=c(-70,-60),ylim=c(-35,-10))
points(cbind(J_pauciflora$longitude, J_pauciflora$latitude), col="red", pch=20, cex=.8)
points(cbind(J_polita$longitude, J_polita$latitude), col="green", pch=20, cex=.8)
...
```{r}
plot(all.worldclim[["bio1"]], xlim=c(-85,-35),ylim=c(-40,12))
points(cbind(J_pauciflora$longitude, J_pauciflora$latitude), col="red", pch=20, cex=.8)
points(cbind(J_polita$longitude, J_polita$latitude), col="green", pch=20, cex=.8)
...
#How can you find ou  t what family it belongs to?
...
#Potentially you could read in a dataframe from elsewhere (csv file, for example).
#And you could submit the variable species names are stored
classification(c("Vanilla planifolia", "Malus pumila"), db="itis")
```
4. For at least one genus, look up all the species in that genus.
```{r}
#If you wanted to know all the genera in a family, or all species in a genus, 
#then you're referring to taxa "downstream"
downstream("Malus", downto="species", db="itis")
```
...
#you need to look at the console to choose the correct taxon of interest
#INTERACTIVE feature will require you to go to the console to scroll and select suggestions
children("pumila", db="col")
```
WORK WITH RGBIF OR SPOCC
6. For both, search and download species occurrence data; prepare as a data-frame
```{r}
library(rgbif)
Malus <- occ_search(scientificName="Malus pumila", limit=1000)
#head(Malus)
colnames(Malus$data)
#Acer$data$genus
#Acer$data$species
unique(Malus$data$genus)
unique(Malus$data$species)
```
7. Clean-up the data-frame:  remove occurrences with missing data and duplicates

```{r}
#Choose just three variables
Malus.small <- subset(Malus$data, select=c("species", "decimalLatitude",
"decimalLongitude"))
#Rename the columns, make sure you're working with a dataframe
colnames(Malus.small) <- c("species", "lat", "lon")
Malus.small <- as.data.frame(Malus.small)
head(Malus.small)
tail(Malus.small)
#There are often occurrences with missing values to remove 
dim(Malus.small)
Malus.small <- Malus.small[complete.cases(Malus.small),]
dim(Malus.small)
#Also there may be duplicates, the same plant or population observed twice
#Or uploaded twice into the database
Malus.u <- unique(Malus.small)
dim(Malus.u)
```
```{r}
library(rgbif)
Vanilla <- occ_search(scientificName="Vanilla planifolia", limit=1000)
#head(Vanilla)
colnames(Vanilla$data)
#Zea$data$genus
#Zea$data$species
unique(Vanilla$data$genus)
unique(Vanilla$data$species)
```

```{r}
#Choose just three variables
Vanilla.small <- subset(Vanilla$data, select=c("species", "decimalLatitude",
"decimalLongitude"))
#Rename the columns, make sure you're working with a dataframe
colnames(Vanilla.small) <- c("species", "lat", "lon")
Vanilla.small <- as.data.frame(Vanilla.small)
head(Vanilla.small)
tail(Vanilla.small)
#There are often occurrences with missing values to remove 
dim(Vanilla.small)
Vanilla.small <- Vanilla.small[complete.cases(Vanilla.small),]
dim(Vanilla.small)
#Also there may be duplicates, the same plant or population observed twice
#Or uploaded twice into the database
Vanilla.u <- unique(Vanilla.small)
dim(Vanilla.u)
```
...
```{r}
plot(all.worldclim[["bio1"]])
points(Vanilla[,c("longitude", "latitude")], pch = 16, cex = 0.5, col = "red")
```
# I didn't do the peninsula because the apples are only shown to be found in the Americas. 
...
```{r}
Europe.worldalt <- crop(all.worldalt,extent(-10, 50, 35, 80))
plot(Europe.worldalt[["alt"]])
points(Vanilla[,c("longitude", "latitude")], pch = 16, cex = 0.5, col = "red")
```
...
```{r}
appeel <- BIEN_occurrence_species(species = "Malus fusca")
plot(all.worldclim[["bio1"]])
points(appeel[,c("longitude", "latitude")], pch = 16, cex = 0.5, col = "red")
```
11. Then re-map on an appropriately cropped map.
```{r}
NAmerica.worldalt <- crop(all.worldalt, extent(-100, -50, 30, 70))
plot(NAmerica.worldalt[["alt"]])
points(cbind(appeel$longitude, appeel$latitude), col="red", pch=20, cex=.8)
```
ONE LAST THING
12. Don't forget to identify the peninsula that we are working with!
# I'm using the northeast United States/ Southeast Canada. 